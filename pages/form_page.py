...
def generate_technical_questions(client, tech_stack, experience_level, position):
    ...
    # Parse questions from response (without displaying to user)
    questions = []
            ...
            if st.button("✅ Submit & Start Interview", type="primary"):
                if not all([full_name, email, phone, experience, position, location, languages]):
                    ...
                    if not validate_email(email):
                        ...
                        # Create a placeholder for spinner and hide form content
                        placeholder = st.empty()
                        
                        with placeholder.container():
                            st.markdown("""
                            <div style="text-align: center; padding: 3rem; background: linear-gradient(145deg, #f7fafc 0%, #edf2f7 100%); 
                                       border-radius: 20px; margin: 2rem 0; box-shadow: 0 10px 30px rgba(0,0,0,0.1);">
                                <h2 style="color: #4a5568; font-family: 'Poppins', sans-serif;">🔄 Preparing Your Interview</h2>
                                <p style="color: #718096; font-size: 1.1rem; margin: 1rem 0;">
                                    Our AI is crafting personalized technical questions based on your profile...
                                </p>
                                <div style="background: #e2e8f0; border-radius: 15px; padding: 6px; margin: 2rem auto; width: 300px;">
                                    <div style="background: linear-gradient(90deg, #667eea, #764ba2); height: 8px; border-radius: 12px; 
                                               width: 100%; animation: pulse 2s infinite;"></div>
                                </div>
                            </div>
                            <style>
                                @keyframes pulse {
                        ...
                        if len(questions) == NUM_QUESTIONS:
                            ...
                            if success:
                                ...
                                # Clear the placeholder and show success message briefly
                                placeholder.empty()
                                st.success(f"✅ Successfully prepared {len(questions)} personalized questions!")
